...
use auth::idp_config::CarlIdentityProviderConfig;
...
pub async fn create(settings: LoadedConfig) -> Result<()> { //TODO
    ...
    let carl_url = CarlUrl::try_from(&settings.config)?;

    let ca_certificate = Pem::from_config_path("network.tls.ca", &settings.config).await?;
    let oidc_enabled = settings.config.get_bool("network.oidc.enabled").unwrap_or(false);
    let oidc_client_manager = if oidc_enabled {
        let carl_oidc_config = CarlIdentityProviderConfig::try_from(&settings.config)
